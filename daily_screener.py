...
PREVIEW_VOL_PROJ = 1.10 # 預估版量增投影：12:55 全日量尚未走完，盤中量 × 此倍率再比昨量
...
def analyze(frames, preview=False):
    """preview：盤中預估版——20 日均量排除今日未完成量；量增以盤中量×投影倍率估全日量"""
    rows = []
    for t, df in frames.items():
        if len(df) < MIN_BARS:
            continue  # 上市未滿約一年 → 排除
        ...
        # 流動性 / 價格過濾
        avg_vol20 = vol.iloc[-21:-1].mean() if preview else vol.iloc[-20:].mean()
        ...
        vol_today = vol.iloc[-1] * (PREVIEW_VOL_PROJ if preview else 1.0)
        signal = bool(
            c > prev_n_high                          # 三盤過
            and c > o                                # 紅K
            and vol_today > vol.iloc[-2] * SIG_VOL_MULT     # 量增
        )
...
var indFilter = '';     // 強勢族群點擊篩選（空字串=不篩選）
...
document.addEventListener('click', function (e) {
  ...
  else if (b.classList.contains('grp-f')) {
    indFilter = (indFilter === b.dataset.ind) ? '' : b.dataset.ind;
    render();
  }
});
...
def build_html(list_a, names, inds, date_label, final=True, top_groups=None):
    def row_html(r):
        ...
        return (
            f"<tr data-code='{code}' data-sym='{tv_symbol(r.ticker)}' data-ind='{ind}'>"
    ...
    if top_groups:
        items = "・".join(
            f"<b class='grp-f' data-ind='{n}' title='只看{n}'>{n}</b> {c:+.1f}%（{k}檔）"
            for n, c, k in top_groups)
...
<style>
  ...
  * {{ box-sizing:border-box; }}
  ...
  .grp b.grp-f {{ cursor:pointer; user-select:none; }}
  .grp b.grp-f.on {{ color:var(--acc); }}
  ...
  /* 手機版：縮欄距字級、產業標籤獨立一行、族群列換行不撐寬 */
  @media (max-width:520px) {{
    body {{ padding:16px 8px 48px; }}
    table {{ font-size:.82rem; }}
    th, td {{ padding:8px 5px; }}
    td.act {{ width:22px; padding:8px 2px; }}
    .tag {{ display:block; margin:3px 0 0; }}
    a.ylink, .sig, .newtag {{ margin-left:5px; }}
    .grp {{ white-space:normal; flex-basis:100%; margin:6px 0 0; }}
  }}
...
盤中預估版之量增以盤中量×{PREVIEW_VOL_PROJ}投影估全日量，訊號以收盤為準。
進場時機自行看圖判斷。點股名開啟 TradingView。<br>
...
def main():
    ...
    result = analyze(fresh, preview=preview)
    if result.empty:
        print("沒有任何股票通過基本過濾。")
        sys.exit(1)
